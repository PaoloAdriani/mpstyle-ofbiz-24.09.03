...
<fo:block text-align="left" padding-top="-45px">
...
<fo:block font-size="8pt">
  ...
  font-family="sans-serif">${companyName}</fo:block>
    <#if postalAddress?exists>
    <fo:block font-family="Monospace" font-size="10pt">${postalAddress.address1} ${postalAddress.postalCode} ${postalAddress.city} (${postalAddress.stateProvinceGeoId?keep_after("-")}) ${postalAddress.countryGeoId}</fo:block>
    <#if vatNumber?exists>
    <fo:block font-family="Monospace" font-size="10pt">P.IVA ${vatNumber}</fo:block>
    </#if>
    <#else>
    ...
    <fo:list-block provisional-distance-between-starts=".5in">
        ...
        <fo:list-item font-family="Monospace">
            <fo:list-item-label>
                <fo:block font-size="10pt">${uiLabelMap.CommonTelephoneAbbr}:</fo:block>
            </fo:list-item-label>
            <fo:list-item-body start-indent="body-start()">
                <fo:block font-size="10pt"><#if phone.countryCode?exists>${phone.countryCode}-</#if><#if phone.areaCode?exists>${phone.areaCode}-</#if>${phone.contactNumber?if_exists}</fo:block>
            </fo:list-item-body>
        </fo:list-item>
        </#if>
        ...
        <fo:list-item font-family="Monospace">
            <fo:list-item-label>
                <fo:block font-size="10pt">${uiLabelMap.CommonEmail}:</fo:block>
            </fo:list-item-label>
            <fo:list-item-body start-indent="body-start()">
                <fo:block font-size="10pt">${email.infoString?if_exists}</fo:block>
            </fo:list-item-body>
        </fo:list-item>
        </#if>
        <#if website?exists>

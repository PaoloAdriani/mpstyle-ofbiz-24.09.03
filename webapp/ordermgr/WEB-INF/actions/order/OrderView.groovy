...
if (orderHeader) {
	...
	orderEmail = orderReadHelper.getOrderEmailString();
	...
	// PAOLO 09/07/2019 cambiato per visualizzazione proprietà customers
	List<GenericValue> partyContactMechPurposeList=delegator.findList("PartyContactMechPurpose", EntityCondition.makeCondition(UtilMisc.toMap("partyId",partyId, "contactMechPurposeTypeId", "PRIMARY_PHONE")), null, null, null, true);
	...
	if(UtilValidate.isNotEmpty(partyContactMechPurposeList))
	{
		...
		GenericValue partyContactMechPurposeTelecomNumber = EntityUtil.getFirst(partyContactMechPurposeList);
		
		String _contactMechId = (String) partyContactMechPurposeTelecomNumber.get("contactMechId");
		
		GenericValue telecomNumber = delegator.findOne("TelecomNumber",UtilMisc.toMap("contactMechId",_contactMechId),true);
		
		String infoString = telecomNumber.getString("contactNumber");
		
		context.phoneNumber = infoString;	
	}
    ...
    //@giulio retrieving internal name
    internalNameMap = [:]
    
    for(orderItem in orderItemList) {
        oiProductId = orderItem.productId;
       
        oiProduct = delegator.findOne("Product", [ "productId" : oiProductId ], false);
        
        if(oiProduct) {
			            
			internalName = oiProduct.internalName;
            
            //check if there is an internal name on this product 
            if(internalName) {
                internalNameMap[oiProductId] = internalName;
				
            }else {
                /* check if the product is a variant; if yes, then go check if the 
                 * virtual parent has an internal name
                 */
                isVariant = oiProduct.isVariant;
                
                if(isVariant == "Y") {
                    
                    EntityCondition cond = EntityCondition.makeCondition(EntityOperator.AND, EntityCondition.makeCondition("productIdTo", EntityOperator.EQUALS, oiProductId),
                                                                                              EntityCondition.makeCondition("productAssocTypeId", EntityOperator.EQUALS, "PRODUCT_VARIANT"));
                    
                    variantAssoc = delegator.findList("ProductAssoc", cond, null, null, null, false);
                    //filter out outdated values
                    variantAssoc = EntityUtil.filterByDate(variantAssoc);
                    
                    List internalNameList = new ArrayList();
                  
                    for(vassoc in variantAssoc) {
                        
                        parentProduct = delegator.findOne("Product", [ "productId" : vassoc.productId], false);
                        internalName = parentProduct.internalName;
                        if(internalName)
                            internalNameList.add(internalName);
                        
                    }
                    
                    //add the list of internal names to the map
                    internalNameMap[oiProductId] = internalNameList;
    ...
    if(internalNameMap != null) 
        context.internalNameMap = internalNameMap;

                    ...
                    <#-- now show inventory reservation info per line item -->
                        <#if orderItemShipGrpInvResList?? && orderItemShipGrpInvResList?has_content>
                            <#list orderItemShipGrpInvResList as orderItemShipGrpInvRes>
                                ...
                                <#assign invItem = EntityQuery.use(delegator).from("InventoryItem").where("inventoryItemId", inventoryItemId).queryOne() />
                                <#assign invFacilityId = invItem.facilityId! />
                                <#assign facility = EntityQuery.use(delegator).from("Facility").where("facilityId", invFacilityId).queryOne() />

                                <#assign facilityName = facility.facilityName! />
                                <#if !facilityName?? >
                                    <#assign facilityName = facility.description! />
                                </#if>
